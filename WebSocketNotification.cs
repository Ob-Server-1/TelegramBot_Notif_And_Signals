...
public class WebSocketNotification //Класс вебсокера
{
    bool tempBool = true;
    byte a = 0;
    string? Price;
    ...
    public async Task<string> Notif(string? symbol, CancellationToken cancellationToken, string SoundPrice, bool Notif = false) // Конект, отправка, получение ответа
    {
        SoundPrice.Replace(".",",");
        bool LogSoundPrice = double.TryParse(SoundPrice.Replace(".",","), out double LastSoundPrice);
            ...
            while (clientSocket.State == WebSocketState.Open) //Открыто ли соединение (подписка) ?
            {
                ...
                else
                {
                    if (cancellationToken.IsCancellationRequested)
                    {
                        ...
                        return "Операции иссключения были завершены";
                    }
